      ...
      <section className="bg-[#0b1220] pt-40 pb-16">
        <div className="max-w-4xl mx-auto px-6 lg:px-10">
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="px-3 py-1 bg-[#e87722] text-white text-xs font-bold uppercase tracking-widest">{post.category}</span>
          </div>
      ...
      <section className="py-16 bg-[#f5f5f5]">
        <div className="max-w-4xl mx-auto px-6 lg:px-10">
          <div className="bg-[#0b1220] p-10 text-center">
            <p className="text-[#e87722] text-xs font-semibold tracking-[0.3em] uppercase mb-3">Watershed Development Group</p>
            ...
            <Link href="/contact" className="inline-flex items-center gap-3 px-8 py-4 bg-[#e87722] hover:bg-[#d0681a] text-white font-semibold uppercase tracking-wide transition-colors">
          ...
          <div className="max-w-4xl mx-auto px-6 lg:px-10 flex flex-col sm:flex-row justify-between gap-6">
            {prev ? (
              <Link href={`/blog/${prev.slug}`} className="group flex items-start gap-3 max-w-xs">
                <ArrowLeft size={16} className="text-[#e87722] mt-1 shrink-0" />
                <div>
                  <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Previous</p>
                  <p className="text-[#0b1220] font-bold text-sm group-hover:text-[#e87722] transition-colors leading-snug">{prev.title}</p>
                </div>
              </Link>
            ) : <div />}
            {next && (
              <Link href={`/blog/${next.slug}`} className="group flex items-start gap-3 max-w-xs text-right sm:text-right ml-auto">
                <div>
                  ...
                  <p className="text-[#0b1220] font-bold text-sm group-hover:text-[#e87722] transition-colors leading-snug">{next.title}</p>
                </div>
                <ArrowRight size={16} className="text-[#e87722] mt-1 shrink-0" />
              </Link>
            )}
          </div>
        </section>
